...
import { ChevronLeft, UserCog, Factory, Truck, ShieldCheck } from "lucide-react";

type RoleChoice = {
  role: "spv" | "staff_produksi" | "staff_packing_pengiriman" | "super_admin";
  label: string;
  icon: typeof UserCog;
  color: string;
};
          ...
          <div className="grid grid-cols-2 gap-3">
            {ROLE_CHOICES.map((rc) => {
              ...
              return (
                <button
                  key={rc.role}
                  onClick={() => setSelectedRole(rc)}
                  className={`flex flex-col items-center justify-center gap-2 p-5 rounded-2xl border-2 transition-all ${COLOR_MAP[rc.color]}`}
                >
                  <Icon size={28} />
                  <span className="text-sm font-semibold text-gray-700 text-center leading-tight">{rc.label}</span>
                </button>
              );
            })}
          </div>
